package com.note.algo.elementary._10_hash;
...
//		System.out.println(Long.MAX_VALUE);
    ...
    public void printKeyTree() {
        System.out.println("当前映射信息为：");
        for (Iterator<Long> it = keyAndRealNode.keySet().iterator(); it.hasNext(); ) {
            Long lo = it.next();
            System.out.println("hash(" + lo + ")连接到主机->" + keyAndRealNode.get(lo));
        }
    ...
    public void keyToNode(String key) {
        ...
        System.out.println(key + "（hash：" + hash(key) + "）连接到主机->" + realNode);
    }
