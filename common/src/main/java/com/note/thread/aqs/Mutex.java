 ...
 * ReentrantLock/CountDownLatch/Semphore这些同步类的实现方式都差不多，不同的地方就在获取-释放资源的方式tryAcquire-tryRelelase。掌握了这点，AQS的核心便被攻破了！
 */
class Mutex implements Lock, Serializable {
    ...
    private static class Sync extends AbstractQueuedSynchronizer {
        ...
        protected boolean tryRelease(int releases) {
            ...
            if (getState() == 0)// 既然来释放，肯定是已占有状态了。只是为了保险，多层判断！
    ...
    // lock<-->acquire。两者语义一样：获取资源，直到成功才返回。
    public void lock() {
        sync.acquire(1);
    }
